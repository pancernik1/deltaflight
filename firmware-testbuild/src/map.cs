...
public class map
{
   ...
   public static string checkForExceptions(List<int> arr, string _path,
    string ext,string cmd)
    {
        ...
        if(args.Count > 0)
        {
            ...
            if (argNames.Contains(null))
            {
                return "unknown argument found"; 
            }
            else if(argVals.Contains(null))
            {
                return "Couldn't parse argument value (it has to be int)";
            }
            else
            {
                return null;
...
static public void jp2ds(string _path, int offX, int offY)
    {
        ...
        float scaleX = (float)outWidth  / fullWidth;
        float scaleY = (float)outHeight / fullHeight;

        int srcOffX = (int)(offX / scaleX);
        int srcOffY = (int)(offY / scaleY);

        srcOffX = Math.Clamp(srcOffX, 0, fullWidth  - outWidth);
        srcOffY = Math.Clamp(srcOffY, 0, fullHeight - outHeight);

        int srcRegionW = fullWidth  - srcOffX;
        int srcRegionH = fullHeight - srcOffY;
